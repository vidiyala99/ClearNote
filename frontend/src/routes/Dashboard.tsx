    ...
    <div className="min-h-screen bg-slate-50 font-body">
      ...
      <main className="p-8 max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full bg-slate-200 text-slate-600 font-medium tracking-wide">Demo Mode</span>
        </div>
          ...
          {mockVisits.map((visit) => (
            <div key={visit.id} className="p-5 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:border-cyan-200 hover:translate-y-[-1px] transition-all cursor-pointer">
              <div className="flex justify-between items-start">
                <h3 className="font-semibold text-slate-800 text-base">{visit.title}</h3>
                <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${visit.status === 'ready' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800 animate-pulse'}`}>
                  {visit.status === 'ready' ? 'Ready' : 'Processing'}
                </span>
              </div>
              <p className="text-sm text-slate-500 mt-1">{visit.doctor}</p>
